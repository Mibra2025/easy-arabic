...
async function initDatabase() {
  try {
    ...
    // Добавляем поле role_id если его нет
    try {
      await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS role_id INTEGER REFERENCES roles(id) DEFAULT 3');
      console.log('role_id column added or already exists');
    } catch (error) {
      console.log('role_id column might already exist:', error.message);
    }
